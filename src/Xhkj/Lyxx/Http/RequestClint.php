...
class RequestClint
{
	...
	public static function gettoken($methodOrUri,Base $Request)
    {
        if(!self::verification($Request)) {
            return json_encode('appKey 或 appSecret 不能为空');
        }

        $Request->addParam('sign', self::setSign($Request->getAllParam(), $Request->getAppKey(), $Request->getAppSecret()));
        $Request->addParam('AppKey', $Request->getAppKey());
        $Request->addParam('AppSecret', $Request->getAppSecret());

        $serverUrl = self::richRequest($methodOrUri, $Request);

        $headers = array(
            'AppKey' => $Request->paramMap['AppKey'],
            'AppSecret' => $Request->paramMap['AppSecret'],
            'sign'=> $Request->paramMap['sign']
        );
	 ...
	 * @return string
	 */
	public static function setSign($data, $AppKey, $AppSecret)
	{
		ksort($data); // 对参数按照键名进行排序
        $signStr = '';
        foreach ($data as $k => $v) {
            $signStr .= "{$k}={$v}&";
        }
        $signStr .= "AppKey={$AppKey}&AppSecret={$AppSecret}"; // 拼接需要加密的字符串
